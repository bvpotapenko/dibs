...
def claim_refusal(conn, actor, task_ids) -> tuple[Refusal, tuple[str, ...]]
                                                                 ...
                                                                 # (C9) — TAKEN (bundle member held/done: holders named),
                                                                 # GATED (member waits on children: children named),
                                                                 # HAND_FULL (held ids named), WAITING (holders of what the
                                                                 # remaining todo rows wait on), EMPTY — then one names
                                                                 # query per kind from a MappingProxyType keyed by Refusal
...
class Refusal(str, Enum):                                        # every way the board says no; keys of the catalog
    UNKNOWN_TASK, NOT_OWNER, TAKEN, GATED, HAND_FULL, WAITING, EMPTY,
...
6. `queries.py` — `board_snapshot → Board`, `deliver_events` cursor semantics, `resolve_task(conn, raw, verb)`, `claim_refusal` (five kinds; raises need `output.Refusal` + `steer`, so land those two `output` members here, stubs for the rest).
