...
**Rev:** 12 — (r12) precision only, after `ARCHITECTURE.md` §13 step 14 landed: §8 defines *the diff* — the four change lists — so a sync that merely re-caches text is an empty diff, the mtime it read is recorded either way, and two syncs of one edit journal once; §2 re-measured with steps 13–14 in. No decision changed. (r11) the finished package measured and its edges hardened: §2 budget restated by measurement and made a test; §6 — malformed invocations are refusals in the verb's own form, exit codes named, `join` is identity-free, `sync` is the import itself, an unknown `--for` name is refused; §8 — a sync that changes nothing journals nothing; D8, D20 amended. (r10) oversized-bundle refusal: D6, §6 amended — a bundle larger than the hand can never fit, so `claim` refuses it as such and steers to its first member; *hand full* is reserved for a bundle that would fit an empty hand; D24 — a hand-checked line's row is done by the author already in the sync diff, so `verify` previews exactly what `init` creates. (r9) one import path + budgets met by measurement: D24 new; §2 budget restated; §5 SQLite floor + `plan_mtime`; §8 sync is one transaction, ids minted in one pass, cached text refreshed; I5 clarified; §13 id lettering settled. (r8) invocation tracing for debugging: D23 new; §6 env note; §12 + deferred `log --jsonl`. Earlier: (r2) small-context workers: D16–D17; D5, D7, D8, D14, I10. (r3) implementation architecture: D3, §2 budget; `ARCHITECTURE.md`. (r4) parallel plans + skill invocation: D2, D8, D13; D18–D19. (r5) board keys, hand limit, verify: D6, D18–D19 amended; D20–D21 new; §2, §5–§6, §8, §10–§13 touched. (r6) leaves-first prerequisites: D22 new; D6–D7 amended; §5–§6, §8, §10–§13 touched. (r7) author skill `dibs-plan`: D1, D13 amended; §2 budget; §10 split into two skill specs. 
...
**Budget:** a stdlib-only package shipped as a single file `dibs.pyz`; two skill files (`dibs`, `dibs-plan`); one board file per plan. Size is measured in *code lines* — a non-blank line that is neither comment-only nor part of a docstring; the body lines of a multi-line SQL or template string count, because they are the code that decides (C9) — across ≤18 small files. Physical lines run about 1.5× code lines by construction — every member carries a docstring and every SQL statement a placeholder legend (D3) — so they are not the budget. Measured at Rev 12, every module landed and `ARCHITECTURE.md` §13 steps 13–14 in: 1654 code lines (253 of them the interior lines of multi-line SQL and template strings) / 2534 physical, in 17 files; the same counter gives 1599 for the tree Rev 11 measured, before those two steps. The Rev 3 target of ~1000 and its 1500 stop were forecasts made before branching moved into SQL and before every refusal got its own wording; the package that implements every decision below costs what it costs, and a deletion pass found nothing in it that is not a decision on this page. **Hard stop: 1700.** From Rev 11 it is a test, not a hope (`tests/test_architecture.py`, with the member budgets and layering of `ARCHITECTURE.md`): crossing it requires an amendment here naming the decision that pays for it, never a step's side effect.
...
- **D8 — Identity.** Per-session generated `adjective-animal` name plus id `name-NNNN`. IDs are for command input; names are for display. This is confusion resistance (an agent can't hallucinate an id it never saw), **not security** — all sessions share one OS user and could open the DB directly. Preferred flow: the *launcher* mints identity — `export DIBS_AS=$(dibs join)` before starting the session — so small models never handle identity at all, and the env var survives context compaction; `claim`-time minting remains as fallback, and a claim that minted prints the binding to export alongside the id. `join` itself is identity-free: it mints and prints whatever `DIBS_AS` already says, because a launcher's `export DIBS_AS=$(dibs join)` must work in an environment still carrying a previous session's identity — so its output is the bare id and nothing else (no feed, no hint). A *supplied* identity unknown to the resolved board is an addressing error — fail with a steer (it almost always means the wrong board, D18) rather than silently minting; auto-mint happens only when no identity was supplied at all.
...
- **D20 — Opaque board keys.** `init` mints a key like `dibs-7f3a-9c2e` (random hex; the `dibs-` prefix makes every handoff line its own skill trigger), stores it in the board's `meta` table (truth), and records key → absolute plan path in a registry under `~/.local/state/dibs/` — a cache of that truth, written at `init` and rewritten by any board command that finds it missing or pointing elsewhere, so a moved plan heals its key the first time an author reaches it by path. **Workers are handed keys; authors use paths** — the same role split as D5. A key resolves identically from any CWD (killing the relative-path wrong-board footgun) and keeps the plan's location out of worker context: a *groove against drift*, not access control (same honesty as D8 — shell access can find anything; the defense is that the lowest-energy action is `dibs claim`, and unclaimed rogue work stays visible at review per D16). Lost key → `dibs list --plan <path>` reprints it.
...
Every verb accepts identity via `--as <id>` or env `DIBS_AS` (env preferred; see `join`), and a board via `--plan <key or file>` or env `DIBS_BOARD` — board keys or plan paths, resolved per D18/D20; the no-board error steers workers to `cd` or `--plan`, never to `init`. With `DIBS_TRACE` set (non-empty), every invocation — success or failure — additionally appends one trace line (D23). Argument parsing is tolerant and errors steer (D14): fuzzy ID matching, `--task A3` / `--task=A3` / positional all accepted, and every error message ends with the literal command to run next. A malformed invocation — no verb, an unknown verb, a missing `--note`, an unrecognized flag — is a refusal like any other: the parser's one-line message, then `Run:` with the verb's canonical form; never a usage dump, never the parser's own exit code. Exit codes: 0 success; 1 every refusal (steered, I10); 2 the board file could not be read or written — an environment fault, and the steer says to retry. Every response ends with: unseen events for the caller (one line each, capped; overflow says "run `dibs list`"), then the next-expected-verb hint.
...
| `sync` | Reconcile plan ↔ DB per §8 and report the diff. The same import runs automatically and silently before every *other* board verb when the plan's mtime changed; the verb itself always runs it and is the one place its result is shown. | Ambiguities are reported, never guessed silently. A sync that changes nothing journals nothing (§8). |
| `join` | Mint identity and print the bare id — nothing else, whatever `DIBS_AS` already says (D8) — for launcher scripts: `export DIBS_AS=$(dibs join)` — so workers never handle identity. | — |
| `claim [--task ID …]` | Mint identity on first use (fallback when `join` wasn't used); CAS-claim the next *available* task — no open children (D22) — by section affinity, then `seq` (D7), or the exact bundle (atomic, all-or-none, must fit the hand). Returns the id reminder (`you are <id>`; when this call minted the identity, the line also carries the `export DIBS_AS=<id>` to run), task title + body, events, next verb. If the task was previously claimed and reaped, output names the prior claimant: "previously claimed by brave-otter, reaped 20 min ago — verify before redoing." | Lost race → tool auto-picks next (no-arg) or reports which bundle member was taken. Hand full → "finish or drop" steer naming the held task(s) (D6). Bundle larger than the hand → refused as such, naming size and hand, steering to `claim --task <first member>` (D6). Nothing available yet → names what the remaining tasks wait on; retry later or stop if the launcher respawns (D6, D22). Explicit `--task` on a gated parent → refused, open children named. Empty board → "no tasks remain; stop." |
...
| `note "…" [--for <name>]` | Append event; broadcast by default; `--for` takes a name (or an id). | Unknown name → refused before anything is logged, steering to the broadcast form; the names that exist are the ones events show. |
...
**The diff** is the four change lists: new lines, vanished lines, hand-checked imports, regressed lines. A sync whose diff is empty journals nothing and touches no state column: refreshing cached text — a reorder, a reworded body, a renamed heading, a re-nesting — is not a mutation, because the text's truth is the file (D4) and the DB only mirrors it. The mtime the sync read is recorded either way (I9), so the file is not parsed again until it changes. This is the common case, not a corner: the tool's own annotation writes change the plan's mtime but never its text truth, so the next command re-reads the file and finds an empty diff; and when two workers auto-sync the same edit, the first imports it and journals once while the second finds an empty diff and journals nothing. Only a non-empty diff earns a SYNC event — I6 counts mutations, and nothing mutated.
...
- **`note --for` refused (no such name)** → the steer is the broadcast form of the same note; run it. Names are what events show you; a name from another board does not exist here.
