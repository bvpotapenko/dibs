...
A sync whose diff is empty writes nothing and journals nothing. This is the common case, not a corner: the tool's own annotation writes change the plan's mtime but never its text truth, so the next command re-reads the file and finds nothing to do. Only a non-empty diff earns a SYNC event — I6 counts mutations, and nothing mutated.
